...
const showMessage = (message) => {
  ...
  document.querySelector('.toastmessage ul').prepend(toast);
...
const checkGuess = (guess, word) => {
  ...
  wordLetters.forEach((letter, index) => {
    ...
    } else if (guessLetters[index].charCodeAt(0) === letter.charCodeAt(0)-1 || guessLetters[index].charCodeAt(0) === letter.charCodeAt(0)+1) {
      currentRow.querySelector(`li:nth-child(${index + 1})`)
        .setAttribute('data-status', 'oneaway');
    } else if (guessLetters[index].charCodeAt(0) === letter.charCodeAt(0)-2 || guessLetters[index].charCodeAt(0) === letter.charCodeAt(0)+2) {
      currentRow.querySelector(`li:nth-child(${index + 1})`)
        .setAttribute('data-status', 'twoaway');
    }
  });
  ...
  if (noOfCorrectLetters == 5) {
    currentRow.setAttribute('data-animation2', 'jump');
    currentRow.style.animationDelay = "2s";
    currentRow.style.transitionDelay = "3s";
...
const onKeyDown = (key) => {
  ...
  if (key === ENTER_KEY) {
    if (currentWord.length < 5) {
      showMessage('Your word must contain 5 letters.');
      return;
    }

    if (currentWord.length === 5 && DICTIONARY.includes(currentWord)) {
      checkGuess(currentWord, WORD_OF_THE_DAY);
    } else {
      currentRow.setAttribute('data-animation', 'notvalid');
      showMessage('Your word is not found in our word list.');
    }
...
const generateBoard = (board, rows = 6, columns = 5, keys = [], keyboard = false) => {
  ...
  // All the ENTER, BACK and ALPHABET keys onto the keyboard.
  if (keyboard) {
    ...
    const alphabetKey = document.createElement('li');
    alphabetKey.setAttribute('data-key', ALPHABET_KEY);
    alphabetKey.textContent = ALPHABET_KEY;
    board.lastChild.append(alphabetKey);
  }
}
...
function closePopUpHowToPlay() {
    let popup = document.getElementById("popup-howtoplay");
    popup.classList.remove("open-popup-howtoplay");
}
